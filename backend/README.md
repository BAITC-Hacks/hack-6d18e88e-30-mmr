...
Модуль из `dev` объединён с защищённым AI backend. Аккаунты не меняют
демонстрационный переключатель ролей и браузерный каталог задач.
`/api/ai/analyze` поддерживает OpenAI/NVIDIA, строгую валидацию и локальный fallback.
...
Настройки читаются из `backend/.env` и корневого `.env`, переменные окружения имеют приоритет.
Для публичного размещения обязательны `APP_ENV=production`, серверный `API_ACCESS_TOKEN`,
точные `API_ALLOWED_HOSTS` и HTTPS `API_ALLOWED_ORIGINS`; формы аккаунта должны идти через
настроенный gateway. Токен никогда не передаётся в `VITE_*`. Production требует Secure cookies
и HTTPS `AUTH_PAGE_URL`. Подробности общей защиты — [SECURITY_AUDIT.md](SECURITY_AUDIT.md).
...
Запросы с cookie, меняющие состояние, требуют `Origin` из `API_ALLOWED_ORIGINS` (`CORS_ORIGINS` поддерживается как старое имя в development).
...
не взаимозаменяемы для cookie. Swagger работает после входа на `/account` на том же хосте.
...
Для размещения задай реальные `AUTH_PAGE_URL`, `API_ALLOWED_ORIGINS`, HTTPS и
...
Следующий отдельный этап: AI analyze/build-card, схема ответа, OpenAI/NVIDIA и локальный fallback.
