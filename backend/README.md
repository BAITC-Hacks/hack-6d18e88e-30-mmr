...
отклики или ручной выбор команды. `/api/ai/analyze` пока остаётся прежней заглушкой;
интеграция OpenAI/NVIDIA — следующий самостоятельный этап.
...
Запросы с cookie, меняющие состояние, требуют `Origin` из `CORS_ORIGINS`.
