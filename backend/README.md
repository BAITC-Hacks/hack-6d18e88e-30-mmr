...
Страница аккаунта находится в React: **http://localhost:5173/auth** (также пункт «Аккаунт» в навигации).
...
Настроенный Supabase никогда не переключается на локальную базу при ошибке. В production-сборке вход требует Supabase-конфигурацию; демонстрационный интерфейс задач доступен отдельно.
...
Backend читает сначала `backend/.env`, затем корневой `.env`, не заменяя уже заданные значения окружения; frontend читает `frontend/.env.local`.
...
Оставьте Supabase-параметры пустыми, `MAIL_BACKEND=file`, `AUTH_PAGE_URL=http://localhost:5173/auth`.
...
При настроенном `API_ACCESS_TOKEN` защищённый серверный gateway передаёт его в `X-API-Access-Token`, сохраняя пользовательский `Authorization: Bearer ...`. Gateway должен проверять пользователя и заменять входящий `X-API-Access-Token` своим значением; этот секрет нельзя выдавать браузеру. Без отдельного заголовка API сохраняет прежнюю проверку серверного токена в Bearer. Неверный или повторный `X-API-Access-Token` отклоняется.
...
Полная локальная проверка объединённой платформы из корня: `npm run check`. Она включает backend, HTTP smoke, SQL-миграции в PGlite, тесты интерфейса и аккаунтов, lint и сборку.
...
Конструктор задач, рейтинг, подбор команд и AI работают в общем интерфейсе на `/`. Аккаунт открывается отдельно; кнопка «Открыть демо» возвращает в рабочее пространство. Демонстрационный переключатель ролей не выдаёт серверных прав; данные задач пока хранятся в браузере.

При публичном размещении обязательны `APP_ENV=production`, серверный `API_ACCESS_TOKEN`, точные `API_ALLOWED_HOSTS` и HTTPS `API_ALLOWED_ORIGINS`; небезопасная конфигурация не запускается. Подробные границы защиты и проверок: [SECURITY_AUDIT.md](SECURITY_AUDIT.md). Production API требует отдельного защищённого gateway; в этот merge его развёртывание не входит.
