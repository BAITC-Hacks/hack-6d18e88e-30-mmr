...
def test_health_account_and_existing_ai_contract(client):
    ...
    result = client.post("/api/ai/analyze", json={"draft": "Нужен бот для обработки заказов магазина"})
...
def test_frontend_cors_and_https_cookie(client):
    ...
    settings = replace(client.app.state.settings, cookie_secure=True, allowed_hosts=("example.com",), allowed_origins=("https://example.com",))
