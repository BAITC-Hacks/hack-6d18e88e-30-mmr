...
assert.ok(calculateTeamMatch(seedTasks.find(task => task.id === 'task-retail')!, seedTeams.find(team => team.id === 'team-datalab')!).total >= 50, 'Retail forecasting remains relevant to DataLab');
for (const task of seedTasks) {
