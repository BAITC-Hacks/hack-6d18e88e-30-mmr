...
async function verify() {
  ...
  assert.equal(task(draft.id).rating, 0, 'Unconfirmed fields cannot earn rating points');
  assert.equal(calculateRatingPreview(task(draft.id)).total, 100);
  ...
  assert.equal(task(draft.id).rating, 100);
  ...
  assert.equal(task(draft.id).rating, 0);
  assert.equal(calculateRatingPreview(task(draft.id)).total, 80);
  ...
  console.log('PASS: draft preview and confirmed rating across publish/edit/reconfirm');
