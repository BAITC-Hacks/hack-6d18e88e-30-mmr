...
def test_supabase_identity_uses_user_token_while_ai_requires_gateway_token(tmp_path):
    ...
    with TestClient(app, headers={"Origin": ORIGIN}) as client:
        ...
        with patch("backend.services.supabase_gateway.httpx.request", side_effect=responses) as remote:
        ...
        assert client.get("/api/ai/inspector", headers={"Authorization": "Bearer user-session"}).status_code == 401
        assert client.get("/api/ai/inspector", headers={"Authorization": f"Bearer {API_TOKEN}"}).status_code == 200
        # Missing or wrong account methods never expand the deployment exemption.
        for path in ("/api/auth/login", "/api/auth/me/", "/api/auth/config/extra"):
            assert client.get(path).json()["detail"]["code"] == "UNAUTHORIZED"
        assert client.post("/api/auth/config", json={}).status_code == 401
...
def test_old_local_cookie_does_not_break_supabase_action_tokens(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        token = "a" * 64
        with patch("backend.services.supabase_gateway.httpx.request", return_value=httpx.Response(200, json=True)) as remote:
