...
def main():
    ...
    env = {**os.environ, 'PYTHON_DOTENV_DISABLED': '1', 'OPENAI_API_KEY': '', 'NVIDIA_API_KEY': '',
           ...
           'AUTH_PROVIDER': 'local', 'AUTH_MAIL_FORMAT': 'html',
    ...
    try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise RuntimeError('Isolated smoke server did not accept a loopback health request within 15 seconds') from None
                    time.sleep(0.1)
        ...
        if process.stderr:
            if not completed:
                # This child uses isolated settings and no real credentials.
                diagnostic = process.stderr.read().decode('utf-8', errors='replace').strip()
                if diagnostic:
                    print(diagnostic[-6000:], file=sys.stderr)
