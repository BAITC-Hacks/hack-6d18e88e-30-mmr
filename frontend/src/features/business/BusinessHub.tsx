...
export function BusinessHub({ view }: { view: 'overview' | 'tasks' | 'proposals' }) {
  ...
  const featuredTask = [...published].sort((a, b) => b.rating - a.rating)[0];
  ...
  return <div className="stack workspace-page">
    ...
    {view === 'overview' && <>
      <section className="business-intro">
        <div className="intro-copy"><span className="eyebrow">МЕСТО, ГДЕ ЗАДАЧИ НАХОДЯТ КОМАНДУ</span><h2>Есть задача.<br /><span>Будет решение.</span></h2><p>Ваш опыт бизнеса. Свежий взгляд студентов. Превратите потребность в понятный проект — начните с пары предложений.</p><Button onClick={createTask}>Сформулировать задачу <span aria-hidden="true">↗</span></Button><span className="intro-caption">AI поможет уточнить. Решение останется за вами.</span></div>
        {featuredTask ? <aside className="intro-feature" aria-label="Задача из вашего каталога">
          <div className="intro-feature-top"><span>ИЗ ВАШЕГО КАТАЛОГА</span><span aria-hidden="true">↗</span></div>
          <div className="intro-feature-score"><strong>{featuredTask.rating}</strong><span>/ 100<br />готовность задачи</span></div>
          <div className="intro-feature-meter" aria-hidden="true"><span style={{ width: `${featuredTask.rating}%` }} /></div>
          <div className="intro-feature-body"><ReadinessBadge score={featuredTask.rating} /><h3>{featuredTask.title}</h3><p>{featuredTask.industry} · {proposals.filter((proposal) => proposal.taskId === featuredTask.id).length} откликов</p></div>
          <button className="intro-feature-link" onClick={() => setDetailsId(featuredTask.id)}>Посмотреть карточку <span aria-hidden="true">↗</span></button>
        </aside> : <aside className="intro-feature intro-feature-empty"><span className="eyebrow">ПЕРВЫЙ ШАГ — ЗА ВАМИ</span><strong>0 → 100</strong><p>Дополняйте и подтверждайте карточку. Каждая полезная деталь повышает её готовность.</p><Button onClick={createTask}>Создать первую задачу ↗</Button></aside>}
        <div className="intro-path" aria-label="Три шага к проекту"><div><span>01</span><strong>Опишите задачу</strong></div><div><span>02</span><strong>Выберите команду</strong></div><div><span>03</span><strong>Подтвердите результат</strong></div></div>
      </section>
