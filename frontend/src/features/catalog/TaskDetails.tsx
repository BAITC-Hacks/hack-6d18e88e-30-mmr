...
export function TaskDetails({ task, onClose }: { task: Task; onClose: () => void }) {
  ...
  return <Modal open onClose={onClose} title={task.title} wide>
    ...
    <div className="modal-actions"><Button variant="secondary" onClick={onClose}>Закрыть</Button>{activeRole === 'business' && <Button onClick={() => { onClose(); openTaskEditor(task.id); }}>Редактировать карточку</Button>}{activeRole === 'student' && task.published && <>{ownProposal && <Button variant="secondary" onClick={() => { onClose(); navigate('my-proposals'); }}>Перейти к моему отклику</Button>}<Button onClick={() => setApplying(true)} disabled={!team}>Отправить предложение <span aria-hidden="true">↗</span></Button></>}</div>
